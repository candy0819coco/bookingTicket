...
const TicketOrder = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState("convenientStore");
  ...
  const handleChangePayment = () => {
    if (paymentMethod === "convenientStore") {
      setPaymentMethod("creditCard");
    } else {
      setPaymentMethod("convenientStore");
    }
  };
  ...
  const handleOrderTicket = async () => {
    ...
    await axios({
      ...
      data: { userInfo: userInfo, totalTickets: totalTickets, orderTime, orderStatus, orderPrice, paymentStatus, paymentMethod },
